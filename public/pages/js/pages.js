(function ($) {
    ...
    Pages.prototype.reponsiveTabs = function () {
        ...
        $('[data-init-reponsive-tabs="dropdownfx"]').each(function () {
            ...
            $(select).wrap(
                '<div class="nav-tab-dropdown cs-wrapper full-width d-lg-none d-xl-none d-md-none"></div>'
            );
            new SelectFx(select);
        });
    };
    ...
    Pages.prototype.initHorizontalMenu = function () {
        ...
        function autoHideLi() {
            ...
            var possibleLi =
                parseInt(hMenuRect.width / (liTotalWidth / liCount)) - 1;
...
(function (window) {
    ...
    SelectFx.prototype._navigateOpts = function (dir) {
        ...
        if (
            ...
            this.preSelCurrent =
                dir === "next" ? tmpcurrent + 1 : tmpcurrent - 1;
...
(function ($) {
    ...
    $(document).on("click", ".messenger-list-item", function (e) {
        var data = $(this).data();
        var id = data.contact;
        $("#contacts").addClass("hide");
        $("#conversation-view").removeClass("hide");
        var convoBox = $("#conversation-view");
        ...
        $.ajax({
            ...
            success: (data) => {
                ...
                // const obj = JSON.parse(data.user);
                console.log(data);
                chatBox.html(data.messages);
                // console.log(obj.name);
                // // console.log(obj.user.name);
                convoBox.find("#username").html(data.user.name);
                $("#chat-user-id").val(data.user.id);
                chatBox.animate({ scrollTop: 100000 }, 1000);
                makeSeen(true, id);
                // console.log(data.user.active_status);
                if (data.user.active_status == 1) {
                    $("#user-active").removeClass("hide");
                } else if (data.user.active_status == 0) {
    ...
    $("#message-form-main").on("submit", (e) => {
        e.preventDefault();
        var id = $("#chat-user-id").val();
        sendMessageMain(id);
    });

    $(document).on("click", ".close-conversation", function (e) {
        $("#contacts").removeClass("hide");
        $("#conversation-view").addClass("hide");
        getContacts();
    });
    ...
    //             var convoBox = el.last().find("#conversation");
    //             var chatBox = el.last();
    ...
    //                     console.log(data.user.name);
    //                     // const obj = JSON.parse(data.user);
    //                     convoBox.html(data.messages);
    //                     // console.log(obj.name);
    //                     // console.log(obj.user.name);
    //                     chatBox.find("#username").html(data.user.name);
    //                     convoBox.animate({ scrollTop: 100000 }, 1000);
    //                     makeSeen(true, id);
    //                     console.log(data.user.active_status);
    //                     if (data.user.active_status == 1) {
    //                         console.log("sog");
...
function sendMessageMain(id) {
    temporaryMsgId += 1;
    let tempID = `temp_${temporaryMsgId}`;
    let hasFile = !!$("#upload-attachment").val();
    const inputValue = $.trim($("#chat-input").val());
    if (inputValue.length > 0 || hasFile) {
        ...
        $.ajax({
            ...
            beforeSend: () => {
                ...
                $("#message-form-main").trigger("reset");
                // cancelAttachment();
                // messageInput.focus();
            },
            success: (data) => {
...
(function ($) {
    ...
    var Quickview = function (element, options) {
        ...
        $(this.options.deleteNoteConfirmButton).click(function () {
            var checked = $(_this.options.notes).find(
    ...
    var Sidebar = function (element, options) {
        ...
        $.Pages.getUserAgent() == "desktop" &&
            this.$sidebarMenu.scrollbar({
                ignoreOverlay: false,
                disableBodyScroll:
                    this.$element.data("disableBodyScroll") == true
                        ? true
                        : false,
            });
        ...
        function sidebarMouseLeave(e) {
            var menuClosedCSS =
                _this.css3d == true ? "translate3d(0, 0,0)" : "translate(0, 0)";
